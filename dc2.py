...
def dotConvertor(qc, filename, ancillas):
    # Convert the circuit to DAG
    if isinstance(circuit, QuantumCircuit):
        dag = circuit_to_dag(circuit)
    elif isinstance(circuit, cirq.Circuit):
        dag = _cirq_to_dag(qc, ancillas)
    dot_str = dag_to_dot(dag)  # Convert the DAG to a DOT string
    with open(filename + ".dot", "w") as f:
        f.write(dot_str)
        

def _cirq_to_dag(qc: cirq.Circuit, ancillas: list) -> DAGCircuit:
    # Initialize a Qiskit DAGCircuit object
    dag = DAGCircuit()

    # Create a QuantumRegister with the same number of qubits as in the Cirq circuit
    qubits = [x for x in i, circuit.all_qubits() if not in ancillas]
    ancilas = [x for x in i, circuit.all_qubits() if in ancillas]
    qubit_idx = [x for x in range(len(circuit.all_qubits())) if x not in ancillas]
    qr = QuantumRegister(len(qubits), "q")
    ar = QuantumRegister(len(ancillas), "anciila")
    dag.add_qreg(qr)
    ...
    for idx, q in enumerate(ciorcuit.all_qubits()):
        if idx not in ancillas:
            qubit_mapping[q] = Qubit(qr, qubit_idx.index(idx))
        elif
            qubit_mapping[q] = Qubit(ar, ancilas.index(idx))
    print(qubit_mapping)
    assert False
