...
def qasmConvertor(circuit, path=None):
    ...
    new_qubits = [0]
    for gate in circuit.data:
        if "barrier" not in str(gate) and len(gate[1]) > 3:
...
def dag_to_dot(dag):
    ...
    even_odd_pairs = {x:[] for x in all_anc}
    ...
    for node in dag.topological_op_nodes():
        ...
        if "MCXVChain" in str(node.op):
            ...
            #print(remove_args)
            # print("add a MCXVChain ", target_prefaced)
            print("all ancillas are ", all_anc)
            dot.add_node(pydot.Node(node_name, label="\"MCX\"",
                qubits="\"" + ",".join(target_prefaced)+"\"", matrix=str([[0,1],[1,0]])))
        elif "MCXGray" in str(node.op):
            if int(target_prefaced[-1]) in all_anc:
                if target_prefaced not in even_odd_pairs[int(target_prefaced[-1])]:
                    matrix = [[0,-1],[1,0]]
                    even_odd_pairs[int(target_prefaced[-1])].append(target_prefaced)
                    # even_odd_pairs[int(target_prefaced[-1])] = True
                else:
                    matrix = [[0,1],[-1,0]]
                    even_odd_pairs[int(target_prefaced[-1])].remove(target_prefaced)
                    # even_odd_pairs[int(target_prefaced[-1])] = False
            else:
                matrix = [[0,1],[1,0]]
            dot.add_node(pydot.Node(node_name, label="\"MCX\"",
                qubits="\"" + ",".join(target_prefaced)+"\"", matrix=str(matrix)))
        
        elif "RCCX" in str(node.op) or "rccx" in str(node.op.name):
            print("RCCX ")
            assert False
            dot.add_node(pydot.Node(node_name, label="\""+str(node.op)+"\"",
                qubits="\"" + ",".join(target_prefaced)+"\"", matrix=str([[0,1],[-1,0]])))
        elif "CCX" in str(node.op):
            assert len(target_prefaced) == 3
            ...
            pass
        elif "CRYGate" in str(node.op):
            # print(node.op, node.op.params)
            theta = node.op.params[0]
            matrix = [[cmath.cos(theta/2), -cmath.sin(theta/2)],
                      [cmath.sin(theta/2), cmath.cos(theta/2)]]
            dot.add_node(pydot.Node(node_name, label="\""+str(node.op)+"\"",
                qubits="\"" + ",".join(target_prefaced)+"\"", matrix=str(matrix)))
            # print(pydot.Node(node_name, label="\""+str(node.op)+"\"", qubits="\"" + ",".join(target_prefaced)+"\"", matrix=str(matrix)))
            # assert False
            ...
            dot.add_node(pydot.Node(node_name, label="\""+str(node.op)+"\"",
                qubits="\"" + ",".join(target_prefaced)+"\"", matrix=str(matrix)))
        elif "U2Gate" in str(node.op):
            phi, lamda = node.op.params
            theta = 0
            matrix = [[(1/cmath.sqrt(2)), -(1/cmath.sqrt(2)) * cmath.exp(1j * lamda)],
                      [(1/cmath.sqrt(2))*cmath.exp(1j * phi), (1/cmath.sqrt(2)) * cmath.exp(1j * (lamda+phi))]]
            dot.add_node(pydot.Node(node_name, label="\""+str(node.op)+"\"",
                qubits="\"" + ",".join(target_prefaced)+"\"", matrix=str(matrix)))
            ...
            print("rccx" in str(node.op.name))
            print("\n\n\n\n\n")
